...
const SmartShelf: React.FC<SmartShelfProps> = ({ products, onRemoveProduct, onScanNew, userProfile }) => {
  ...
  const renderActionPlan = () => {
      ...
      const hasActions = analysis.riskyProducts.length > 0 || analysis.conflicts.length > 0 || analysis.missing.length > 0 || analysis.redundancies.length > 0;
      ...
      return (
          <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-700">
               ...
               {analysis.conflicts.map((conflict, i) => (
                   <div key={`conflict-${i}`} className="flex items-start gap-4 p-4 rounded-[1.5rem] bg-indigo-50 border border-indigo-100 relative overflow-hidden">
                       ...
                       <div className="flex-1">
                           <h4 className="text-xs font-black uppercase tracking-wide text-indigo-800 mb-1">Adjust Routine</h4>
             ...
             <div className="relative z-10 flex items-start justify-between">
                 <div>
                     ...
                     <p className="text-xs font-medium text-zinc-500 mt-2 max-w-[180px] leading-relaxed">
                         {shelfIQ.analysis.grade === 'S' ? "Clinical perfection. Optimized for efficacy." :
                          shelfIQ.analysis.grade === 'A' ? "Highly effective with minor tweaks needed." :
                          shelfIQ.analysis.grade === 'B' ? "Solid foundation but lacks targeting." :
       ...
       {selectedProduct && (
           <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-zinc-900/40 backdrop-blur-md animate-in fade-in">
                <div className="w-full max-w-sm bg-white rounded-[2.5rem] p-8 relative animate-in zoom-in-95 shadow-2xl overflow-y-auto max-h-[90vh] no-scrollbar">
                    <button onClick={() => setSelectedProduct(null)} className="absolute top-6 right-6 p-2 bg-zinc-50 rounded-full text-zinc-400 hover:bg-zinc-100 transition-colors">
                        <X size={20} />
                    </button>
                    ...
                    <div className="space-y-6">
                                 ...
                                 <div className={`p-6 rounded-[1.5rem] border ${cardStyle}`}>
                                     ...
                                     {warning ? (
                                         <p className="text-sm text-rose-800 font-medium leading-relaxed">{audit.warnings[0].reason}</p>
                                          ...
                                          <p className="text-sm text-amber-800 font-medium leading-relaxed">This product doesn't strongly target your specific skin needs (Score {score}).</p>
                                     ) : (
                        ...
                        {auditProduct(selectedProduct, userProfile).warnings.length === 0 && selectedProduct.benefits.length > 0 && (
                            <div className="p-6 rounded-[1.5rem] bg-teal-50 border border-teal-100">
                                <h4 className="text-xs font-bold text-teal-900 uppercase tracking-widest mb-4 flex items-center gap-2">
                                    <Sparkles size={14} /> Targets Your Needs
                                </h4>
                                <ul className="space-y-3">
                                    {getSortedBenefits(selectedProduct).map((b, i) => {
                                         const userScore = userProfile.biometrics[b.target as keyof SkinMetrics] as number || 0;
                                         const isCritical = userScore < 50;
                                         const isNeeded = userScore >= 50 && userScore < 75;
                                         
                                         return (
                                             <li key={i} className="flex items-start gap-3 bg-white p-3 rounded-2xl border border-teal-100 shadow-sm">
                                                 <div className={`mt-0.5 shrink-0`}>
                                                     {isCritical ? <AlertTriangle size={16} className="text-rose-500" /> : 
                                                      isNeeded ? <Zap size={16} className="text-amber-500" /> :
                                                      <ShieldCheck size={16} className="text-emerald-500" />}
                                                 </div>
                                                 
                                                 <div className="flex-1">
                                                     <div className="flex justify-between items-start mb-1">
                                                         <span className="text-xs font-bold text-teal-950">{b.ingredient}</span>
                                                         <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-md tracking-wide ${
                                                             isCritical ? 'bg-rose-100 text-rose-600' :
                                                             isNeeded ? 'bg-amber-100 text-amber-600' :
                                                             'bg-emerald-100 text-emerald-600'
                                                         }`}>
                                                             {isCritical ? 'Critical' : isNeeded ? 'Boost' : 'Sustain'}
                                                         </span>
                                                     </div>
